...
export default function MainLayout() {
  ...
  // ✅ Demo user detection (edit this rule if you prefer a specific email)
  // e.g. stricter:
  const demoAllowed = user?.email?.toLowerCase() === "demo@finnexa.com";

  // Optional: close sidebar after nav click on mobile
    ...
    <div className="min-h-screen flex flex-col md:flex-row bg-slate-200 dark:bg-[#07090E] transition-colors duration-500">
      ...
      >
        ...
        <nav className="flex flex-col space-y-2">
          ...
          {/* ✅ Demo Tools (only for demo users) */}
          {demoAllowed && (
            <Link
              to="/demo-tools"
              onClick={closeMobileSidebar}
              className="group flex items-center gap-4 px-4 py-3 rounded-2xl transition-all hover:bg-white/10"
            >
              <span className="w-3 h-3 rounded-full bg-indigo-400 shadow-[0_0_10px_rgba(129,140,248,0.6)]"></span>
              <span className="text-sm font-black uppercase tracking-widest text-slate-200 group-hover:text-white">
                Demo Tools
              </span>
            </Link>
